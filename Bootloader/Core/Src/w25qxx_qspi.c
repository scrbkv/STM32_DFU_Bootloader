...
void W25q_Read_4k(uint32_t address, uint8_t* buffer) {
	uint8_t result;
	...
	if(result == w25qxx_OK)
		result = HAL_QSPI_Receive(&hqspi, buffer, HAL_QPSI_TIMEOUT_DEFAULT_VALUE);
}

void W25q_Write_4k(uint32_t address, const uint8_t* buffer) {

	for (int i = 0; i < 16; ++i, buffer += 256, address += 256) {
		uint8_t result;
		W25qxx_WriteEnable();
		result = QSPI_Send_CMD(&hqspi, W25X_PageProgram, address, QSPI_ADDRESS_24_BITS, 0, QSPI_INSTRUCTION_4_LINES, QSPI_ADDRESS_4_LINES, QSPI_DATA_4_LINES, 256);
		if(result == w25qxx_OK)
			result = HAL_QSPI_Transmit(&hqspi, buffer, HAL_QPSI_TIMEOUT_DEFAULT_VALUE);
		if(result == w25qxx_OK)
			W25QXX_Wait_Busy();
	}
}

void W25q_Erase_4k(uint32_t address) {
	uint8_t result;
	W25qxx_WriteEnable();
	W25QXX_Wait_Busy();
	result = QSPI_Send_CMD(&hqspi, W25X_SectorErase, address, QSPI_ADDRESS_24_BITS, 0, QSPI_INSTRUCTION_4_LINES, QSPI_ADDRESS_4_LINES, QSPI_DATA_NONE, 0);
	if(result == w25qxx_OK)
		W25QXX_Wait_Busy();
}
